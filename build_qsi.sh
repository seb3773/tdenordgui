...
# ==============================================================================
# tdeNordgui - Q4OS Installer (.qsi) Builder
# ==============================================================================
...
echo "=================================================="
echo " tdeNordgui - Q4OS .qsi Package Builder"
echo "=================================================="
...
find_latest_deb() {
    local debs=("$SCRIPT_DIR"/tdenordgui_[0-9]*_*.deb "$SCRIPT_DIR"/*.deb)
    for f in "${debs[@]}"; do
        if [ -f "$f" ]; then
            echo "$f"
            return 0
...
PACKAGE_NAME=$(dpkg-deb -f "$LATEST_DEB" Package 2>/dev/null || echo "tdenordgui")
...
APPNAME_DESC="NordVPN GUI for Trinity"
APP_ICON="tdenordgui"
PK_VERS="$APP_VERSION"
SETUP_TYPE="2"
